...
const GoogleOAuthButtons = () => {
  ...
  const onGoogleSignIn = async (e) => {
    ...
    if (!isSigningIn) {
      setIsSigningIn(true);
      try {
        await doSignInWithGoogle(); // Attempt to sign in
        // Handle success (e.g., navigate to the dashboard, show a message, etc.)
  ...
  return (
    <>
      <button
        ...
        className={`w-full flex items-center justify-center gap-x-3 p-2.5 border rounded-lg text-sm font-medium bg-swatch-2/50  ${
          isSigningIn
            ? "cursor-not-allowed"
      ...
      >
        ...
        {isSigningIn ? (
          <div className="inline-flex items-center gap-2">
            Signing In
            <Loader className="size-5 text-purple-200 animate-spin" />
          </div>
        ) : (
...
export default GoogleOAuthButtons;
